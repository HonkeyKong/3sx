...
#elif CRS_INPUT_DRIVER_LIBRETRO
...
#elif CRS_INPUT_DRIVER_LIBRETRO
    return LibretroInput_IsGamepadConnected(id);
#else
    return false;
#endif
}
...
#elif CRS_INPUT_DRIVER_LIBRETRO
    LibretroInput_GetButtonState(id, state);
#endif
}
...
#elif CRS_INPUT_DRIVER_LIBRETRO
    (void)id;
    (void)low_freq_enabled;
    (void)high_freq_rumble;
#endif
}
