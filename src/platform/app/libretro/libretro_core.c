...
RETRO_API void retro_set_environment(retro_environment_t cb){environ_cb=cb;bool no=false;cb(RETRO_ENVIRONMENT_SET_SUPPORT_NO_GAME,&no);}
...
RETRO_API void retro_deinit(void){if(loaded)retro_unload_game();environ_cb=NULL;log_cb=fallback_log;}
...
RETRO_API void retro_unload_game(void){if(!loaded)return;Main_Shutdown();AFS_Finish();Config_Destroy();LibretroRenderer_Quit();LibretroInput_Reset();Resources_Reset();Paths_ResetOverrides();loaded=false;}
RETRO_API void retro_run(void){if(!loaded||!LibretroRenderer_ContextReady())return;poll_input();AFS_RunServer();Main_StepFrame();ADX_ProcessTracks();LibretroRenderer_Present();Main_FinishFrame();if(video_cb)video_cb(RETRO_HW_FRAME_BUFFER_VALID,384,224,0);double exact=RATE/FPS+audio_remainder;unsigned frames=(unsigned)exact;audio_remainder=exact-frames;if(frames>2048)frames=2048;SPU_RenderSamples(audio_buffer,frames);if(audio_batch_cb)audio_batch_cb(audio_buffer,frames);else if(audio_cb)for(unsigned i=0;i<frames;i++)audio_cb(audio_buffer[i*2],audio_buffer[i*2+1]);}
