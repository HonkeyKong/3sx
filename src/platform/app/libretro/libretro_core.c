...
RETRO_API void retro_deinit(void){if(loaded)retro_unload_game();sx3_ext_game_unloaded();LibretroOnlineStart_Reset();Log_SetCallback(NULL);environ_cb=NULL;}
...
RETRO_API bool retro_unserialize(const void*d,size_t s){if(!loaded)return false;RollbackStateKind kind=RollbackState_GetKind(d,s);if(!kind||(kind==ROLLBACK_STATE_FULL)!=mpp_w.inGame)return false;bool ok=RollbackState_Load(d,s);if(ok)sx3_ext_state_restored();return ok;}
